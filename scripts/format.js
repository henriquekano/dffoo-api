...
const { relateCommandToCharacter, deindexIndexedObjects } = require('./helpers')
...
const renameKeys = R.curry((keysMap, obj) =>
  R.reduce((acc, key) => R.assoc(R.type(keysMap[key]) === 'Function'
    ? keysMap[key](key, obj)
    : (keysMap[key] || key),
    obj[key],
    acc),
...
module.exports = async ({
    ...
    deindexArray(commands, 'character_slug').map((passive) => ({
      ...
      attr: {
        ...
        ele_attr: passive.attr.ele_attr.map((elementAttrIndex) =>
          ...
          elementWithId(enums.elements, elementAttrIndex - 1).name,
        ),
      },
    })),
    characters,
  ...
  passives: [
    ...
    ...R.pipe(
      ...
      (formattedArtifactPassives) =>
        formattedArtifactPassives.map((artifactPassive) => {
          if (!artifactPassive.character_slug) {
            const similarArtifact = formattedArtifactPassives.find((filteredArtPass) =>
    ...
    resist: {
      ...
      ail: {
        ...
        weak: enemy.resist.ail.weak.map((value) =>
          enemies.ailmentResistNames[value - 1],
